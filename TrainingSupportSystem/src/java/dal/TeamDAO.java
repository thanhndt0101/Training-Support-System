...
public class TeamDAO extends DBContext<Object> {
    ...
    public List<Team> getAllTeamByMilestone(String milestone) {
        List<Team> list = new ArrayList<>();
        String sql = "SELECT * FROM team inner join submit where team.team_id = submit.team_id and submit.milestone_⁯id = ?;";
        try {
            PreparedStatement st = conection.prepareStatement(sql);
    ...
    public void inserTeamUpload(int team_id, String class_id) {
    ...
    }public Boolean CheckteamInSubmit(int team_id) {

        
            String sql = "select * from submit where team_id =?";
            try {
                PreparedStatement st = conection.prepareStatement(sql);
                st.setInt(1, team_id);
                ResultSet rs = st.executeQuery();
    ...
    public void insertsubmitforupload(int team_id, String class_id, String milestone) {
...
//            stm.setString(1, class_id);
    ...
    public String getTeamidOfWT(String milestone) {
        String sql = "SELECT * FROM team inner join submit on team.team_id = submit.team_id where submit.milestone_⁯id = ? and team.team_code = 'Waiting list';";
        try {
            PreparedStatement st = conection.prepareStatement(sql);
            st.setString(1, milestone);
            ResultSet rs = st.executeQuery();
    ...
    public void EditTeamId(String team_id, String user_id,String team_old) {
        try {
            String sql = "UPDATE `team_member` SET `team_id` = ?,`is_leader` = 0, `is_active` = 1 WHERE (`user_id` = ?) and (`team_id` = ?);";
            PreparedStatement stm = conection.prepareStatement(sql);
            stm.setString(1, team_id);
            stm.setString(2, user_id);
            stm.setString(3, team_old);
            stm.executeUpdate();
        } catch (Exception e) {
            System.out.println(e);
        }
    ...
    public void UpdateLeader(String user_id, String team_id) {
        try {
            String sql = "UPDATE `team_member` SET `is_leader` = '1' WHERE (`user_id` = ?) and (`team_id` = ?);";
            PreparedStatement stm = conection.prepareStatement(sql);
            stm.setString(1, user_id);
            stm.setString(2, team_id);
    ...
    public String getTeamOldByMileStone(String milestone_id,String user_id) {
        String sql = "SELECT * FROM team_member inner join submit where submit.team_id = team_member.team_id and team_member.user_id =? and submit.milestone_⁯id = ?";
        try {
            PreparedStatement stm = conection.prepareStatement(sql);
            stm.setString(1, user_id);
            stm.setString(2, milestone_id);
            ResultSet rs = stm.executeQuery();
            if (rs.next()) {
               return rs.getString("team_id");
    ...
    public List<Milestone> getAllMileStoneByClassId(String class_id) {
        List<Milestone> list = new ArrayList<>();
    ...
    public List<String> getTeamByMilestone(String mileStone) {
        List<String> list = new ArrayList<>();
        try {
            String sql = "SELECT * FROM submit where milestone_⁯id = ?";
            PreparedStatement stm = conection.prepareStatement(sql);
            stm.setString(1, mileStone);
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
                String test = rs.getString("team_id");
                list.add(test);
            }
    ...
    public List<String> getSubmitBymilestone(String mileStone) {
        List<String> list = new ArrayList<>();
        try {
            String sql = "SELECT * FROM submit where milestone_⁯id = ?";
            PreparedStatement stm = conection.prepareStatement(sql);
            ...
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
                String test = rs.getString("submit_id");
                list.add(test);
            }
    ...
    public String getOneSubmitBymilestone(String mileStone,String team_id) {
        List<String> list = new ArrayList<>();
        try {
            String sql = "SELECT * FROM submit where milestone_⁯id = ? and team_id = ?";
            PreparedStatement stm = conection.prepareStatement(sql);
            stm.setString(1, mileStone);
            stm.setString(2, team_id);
            ResultSet rs = stm.executeQuery();
            if (rs.next()) {
                 return rs.getString("submit_id");
    ...
    public List<String> getUserByTeam(String team_id) {
        List<String> list = new ArrayList<>();
        try {
    ...
    public void deleteSubmit(int submit_id) {
        try {
            String sql = "DELETE FROM `submit` WHERE (`submit_id` = ?) ";
            PreparedStatement stm = conection.prepareStatement(sql);
           stm.setInt(1, submit_id);
    ...
    public void deleteTeamMember(Team_member t) {
        try {
            String sql = "DELETE FROM `team_member` WHERE (`team_id` = ?) and (`user_id` = ?);";
            PreparedStatement stm = conection.prepareStatement(sql);
            stm.setInt(1, t.getTeam().getTeam_id());
            stm.setInt(2, t.getUser().getUser_id());
