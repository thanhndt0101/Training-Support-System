...
//        List<User> list = t.getAllTrainee(class_id);
        ...
        if (total % group == 0) {
            ...
            for (int j = 0; j < (total % group); j++) {
                ...
                list2.get(group ).getListTeamMember().add(m);
                list2.get(group ).setTeam_id(countTeam);
                count++;
            }
        }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        t.insertsubmit(1, class_id, milestone);
        ...
        if (total % group == 0) {
            ...
            for (int j = 0; j < ((total % group)); j++) {
                    ...
                    t.inserTeamMember(list2.get(list2.size()-1).getListTeamMember().get(j).getTeam().getTeam_id(),
                            list2.get(list2.size()-1).getListTeamMember().get(j).getUser().getUser_id(),"0");
                }
