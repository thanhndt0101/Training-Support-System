...
public class TraineeListController extends HttpServlet {
    ...
    protected void listTrainee(HttpServletRequest request, HttpServletResponse response)
        ...
        request.setAttribute("class_user_list", cDB.list());
        ...
        int page_size = 5;
        int count = clDB.count(class_id, status, class_user);
        ...
        ArrayList<Class_user> class_user_list = clDB.paggingV2(page_size, page_index, class_id, status, class_user);
...
//        }
        ...
        request.setAttribute("class_user", class_user);
        response.sendRedirect("traineelist?action=view&class_get=" + cu.getClass_id().getClass_id());
    }
